...
from config.types import TLightConfig
...
class LightManager:
  def __init__(self, device_name: str, initial_brightness: int, starting_color="red"):
    ...
    self.power_status: Literal["on", "off"] = "off"
  ...
  def __create_light_config(self, device_name: str):
    ...
    if not light_config:
      raise Exception(
        f"device with name {device_name} not found"
      )
  ...
  def __initialize_light(self, starting_color, initial_brightness):
    ...
    increase_brightness_commands = (["decrease_brightness"]*self.light_config["max_brightness"] + 
                                    ["increase_brightness"]*initial_brightness)
    commands = ["on", starting_color] + increase_brightness_commands + ["off"]
    self.__execute_commands_internal(commands)
  ...
  def __execute_commands_internal(self, commands: list[str]):
    ...
    for command_to_exec in commands:
      ...
      response = requests.request("POST", SEND_PACKET_ENDPOINT, headers=headers, data=payload)
